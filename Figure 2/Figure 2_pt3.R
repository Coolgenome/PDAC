setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient12_3")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient12_3/" 
...
png(paste0("./Figure 2_pt3_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_12_p105.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 3751)
Node2 <- SelectingSubTreeData(my.subtrees, 2766)
Node3 <- SelectingSubTreeData(my.subtrees, 2345)
Node4 <- SelectingSubTreeData(my.subtrees, 1840)
Node5 <- SelectingSubTreeData(my.subtrees, 1197)
Node6 <- SelectingSubTreeData(my.subtrees, 984)
Node7 <- SelectingSubTreeData(my.subtrees, 748)
Node8 <- SelectingSubTreeData(my.subtrees, 225)
Node9 <- SelectingSubTreeData(my.subtrees, 4)												  

#Merging All Nodes Together
MergedNodes <- rbind(Node1, Node2, Node3, Node4, Node5, Node6, Node7, Node8, Node9)

#Then drop the Histology column
colnames(MergedNodes) = c("Barcode", "Node")

#This then renames "Node" to "Clone"
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_3751", "Clone B2",
					 ifelse(MergedNodes$Node == "Node_2766" , "Clone B3",
                     ifelse(MergedNodes$Node == "Node_2345" , "Clone B1",
					 ifelse(MergedNodes$Node == "Node_1840" , "Clone A2", 
					 ifelse(MergedNodes$Node == "Node_1197" , "Clone A1", 
					 ifelse(MergedNodes$Node == "Node_984" , "Clone C4", 
					 ifelse(MergedNodes$Node == "Node_748" , "Clone C3", 
					 ifelse(MergedNodes$Node == "Node_225" , "Clone C2", 
                     ifelse(MergedNodes$Node == "Node_4" , "Clone C1", MergedNodes$Node)))))))))


#Include clone information in the metadata.
ST_case_12@meta.data$Histology_anno_clone <- ST_case_12@meta.data$Histology_anno <- "NT"
ST_case_12@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_12@meta.data))] <- MergedNodes$Node
ST_case_12@meta.data$Clone <- ST_case_12@meta.data$Histology_anno_clone 
ST_case_12@meta.data$Clone[which(ST_case_12@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_12@meta.data$orig.ident <- gsub("Sample105_", "Pt-3", ST_case_12@meta.data$orig.ident)
ST_case_12@meta.data$site[ST_case_12@meta.data$site == "Panc"] <- "Pri"
ST_case_12@meta.data$site[ST_case_12@meta.data$site == "Liver"] <- "LiM"
ST_case_12@meta.data$site[ST_case_12@meta.data$site == "Lung"] <- "LuM"
ST_case_12@meta.data$site[ST_case_12@meta.data$site == "PC"] <- "PerM"
ST_case_12@meta.data$site[ST_case_12@meta.data$orig.ident == "Pt-3A"] <- "Pri"
ST_case_12@meta.data$site[ST_case_12@meta.data$orig.ident == "Pt-3B"] <- "LiM"
ST_case_12@meta.data$site[ST_case_12@meta.data$orig.ident == "Pt-3C"] <- "PerM"
names(ST_case_12@images) <- c(names(table(paste(ST_case_12@meta.data$orig.ident, ST_case_12@meta.data$site))))
Idents(ST_case_12) <- factor(ST_case_12@meta.data$Clone, levels = sort(names(table(ST_case_12@meta.data$Clone))) )
...
clone.color <- table(ST_case_12@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_12$Clone)))
...
gp1 <- flip(gp1, 5463, 6611)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 6107, 5464)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 8018, 7033)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 5014, 4270)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 5251, 5015)

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_12@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <-  c(6107, 5464, 8018, 7033,  6612, 5251,  5015, 4492, 4271)
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient12_3/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt3_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient12_3/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_12@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_12@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_12@meta.data$Clone, ST_case_12@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt3_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt3_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_12_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_12_tumor <- subset(ST_case_12, idents = names(table(Idents(ST_case_12)))[ grep("Clone", names(table(Idents(ST_case_12))))  ])
ST_case_12_tumor_cnv <- ST_case_12_tumor_cnv[match(rownames(ST_case_12_tumor@meta.data), names(ST_case_12_tumor_cnv))]
length(names(ST_case_12_tumor_cnv) == rownames(ST_case_12_tumor@meta.data))
ST_case_12_tumor@meta.data$InferCNV_score <- ST_case_12_tumor_cnv

Idents(ST_case_12_tumor) <- factor(Idents(ST_case_12_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_12_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt3_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt3_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_12, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
