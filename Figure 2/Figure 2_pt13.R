setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient4_13")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient4_13/" 
...
png(paste0("./Figure 2_pt13_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_4_p137.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 7656)
Node2 <- SelectingSubTreeData(my.subtrees, 6915)
Node3 <- SelectingSubTreeData(my.subtrees, 6583)
Node4 <- SelectingSubTreeData(my.subtrees, 6091)
Node5 <- SelectingSubTreeData(my.subtrees, 5511)
Node6 <- SelectingSubTreeData(my.subtrees, 5214)
Node7 <- SelectingSubTreeData(my.subtrees, 4665)
Node8 <- SelectingSubTreeData(my.subtrees, 3749)
Node9 <- SelectingSubTreeData(my.subtrees, 2451)												  				
Node10 <- SelectingSubTreeData(my.subtrees, 1525)			
Node11 <- SelectingSubTreeData(my.subtrees, 3)	

#Merging All Nodes Together
MergedNodes <- rbind(Node1, Node2, Node3, Node4, Node5, Node6, Node7, Node8, Node9, Node10, Node11)

#Then drop the Histology column
colnames(MergedNodes) = c("Barcode", "Node")

#This then renames "Node" to "Clone"
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_7656", "Clone A2",
					 ifelse(MergedNodes$Node == "Node_6915" , "Clone A1",
                     ifelse(MergedNodes$Node == "Node_6583" , "Clone A4",
					 ifelse(MergedNodes$Node == "Node_6091" , "Clone A3", 
					 ifelse(MergedNodes$Node == "Node_5511" , "Clone B1", 
					 ifelse(MergedNodes$Node == "Node_5214" , "Clone B2", 
					 ifelse(MergedNodes$Node == "Node_4665" , "Clone C1", 
					 ifelse(MergedNodes$Node == "Node_3749" , "Clone C2", 
					  ifelse(MergedNodes$Node == "Node_2451" , "Clone D2", 
					   ifelse(MergedNodes$Node == "Node_1525" , "Clone D3", 
                     ifelse(MergedNodes$Node == "Node_3" , "Clone D1", MergedNodes$Node)))))))))))

#Include clone information in the metadata.
ST_case_4@meta.data$Histology_anno_clone <- ST_case_4@meta.data$Histology_anno <- "NT"
ST_case_4@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_4@meta.data))] <- MergedNodes$Node
ST_case_4@meta.data$Clone <- ST_case_4@meta.data$Histology_anno_clone 
ST_case_4@meta.data$Clone[which(ST_case_4@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_4@meta.data$orig.ident <- gsub("Sample137_", "Pt-13", ST_case_4@meta.data$orig.ident)
ST_case_4@meta.data$orig.ident <- gsub("Sample137D366D", "Pt-13E", ST_case_4@meta.data$orig.ident)
ST_case_4@meta.data$site[ST_case_4@meta.data$site == "Panc"] <- "Pri"
ST_case_4@meta.data$site[ST_case_4@meta.data$site == "Liver"] <- "LiM"
ST_case_4@meta.data$site[ST_case_4@meta.data$site == "Lung"] <- "LuM"
ST_case_4@meta.data$site[ST_case_4@meta.data$site == "PC"] <- "PerM"
ST_case_4@meta.data$site[ST_case_4@meta.data$orig.ident == "Pt-13A"] <- "Pri"
ST_case_4@meta.data$site[ST_case_4@meta.data$orig.ident == "Pt-13B"] <- "LiM"
ST_case_4@meta.data$site[ST_case_4@meta.data$orig.ident == "Pt-13C"] <- "LuM"
ST_case_4@meta.data$site[ST_case_4@meta.data$orig.ident == "Pt-13D"] <- "PerM-1"
ST_case_4@meta.data$site[ST_case_4@meta.data$orig.ident == "Pt-13E"] <- "PerM-2"

names(ST_case_4@images) <- c(names(table(paste(ST_case_4@meta.data$orig.ident, ST_case_4@meta.data$site))))
Idents(ST_case_4) <- factor(ST_case_4@meta.data$Clone, levels = sort(names(table(ST_case_4@meta.data$Clone))) )
...
clone.color <- table(ST_case_4@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_4$Clone)))
...
gp1 <- flip(gp1, 14942, 15683)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 12692, 11776)
gp1 <- flip(gp1, 8029, 11774)
gp1 <- flip(gp1, 11775, 13239)
gp1 <- flip(gp1, 11776, 12692)
gp1 <- flip(gp1, 10478, 9552)
gp1 <- flip(gp1, 13241, 13538)
gp1 <- flip(gp1, 14118, 14609)
gp1 <- flip(gp1, 14941, 14610)

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_4@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <- c(15683, 14942, 14610, 14118,  13538, 13241, 12692, 11776, 10478,  9552, 8030)
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient4_13/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt13_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient4_13/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_4@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_4@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_4@meta.data$Clone, ST_case_4@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt13_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt13_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_4_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_4_tumor <- subset(ST_case_4, idents = names(table(Idents(ST_case_4)))[ grep("Clone", names(table(Idents(ST_case_4))))  ])
ST_case_4_tumor_cnv <- ST_case_4_tumor_cnv[match(rownames(ST_case_4_tumor@meta.data), names(ST_case_4_tumor_cnv))]
length(names(ST_case_4_tumor_cnv) == rownames(ST_case_4_tumor@meta.data))
ST_case_4_tumor@meta.data$InferCNV_score <- ST_case_4_tumor_cnv

Idents(ST_case_4_tumor) <- factor(Idents(ST_case_4_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_4_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt13_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt13_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_4, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()

#----------------------------------------------------------Figure 3

tumor_marker <- read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/three_subtype_marker.txt")

DefaultAssay(ST_case_4) <- "Spatial"
ST_case_4 <- AddModuleScore(ST_case_4, features = list(tumor_marker$scClassical), 	name = "Classical", replace = TRUE)
ST_case_4 <- AddModuleScore(ST_case_4, features = list(tumor_marker$scBasal),       	name = "Basal", replace = TRUE)

ST_case_4_df <- data.frame(cbind(ST_case_4$Classical1, ST_case_4$Basal1, "NT", ST_case_4$Clone))
colnames(ST_case_4_df) <- c("Classical", "Basal", "Lineage", "Clone")

id1 <- which(ST_case_4$Basal1 < ST_case_4$Classical1 - 0.2)	#, which(as.numeric(ST_case_4$Classical1) > 0.2))
id2 <- which(ST_case_4$Basal1 > ST_case_4$Classical1 + 0.2)	#, which(as.numeric(ST_case_4$Basal1) > 0.2))
id3 <- intersect(which(ST_case_4$Basal1 < ST_case_4$Classical1 + 0.2), which(ST_case_4$Basal1 > ST_case_4$Classical1 - 0.2))
ST_case_4_df$Lineage[id1] <- "Classical"
ST_case_4_df$Lineage[id2] <- "Basal"
ST_case_4_df$Lineage[id3] <- "Intermediate"
ST_case_4_df$Lineage[grep("Clone", ST_case_4_df$Clone, invert = T)] <- "NT"

ST_case_4_df_tumor <- ST_case_4_df[grep("Clone", ST_case_4_df$Clone),]
ST_case_4_df_tumor$Classical <- as.numeric(ST_case_4_df_tumor$Classical)
ST_case_4_df_tumor$Basal <- as.numeric(ST_case_4_df_tumor$Basal)
ST_case_4_df_tumor$Lineage <- factor(ST_case_4_df_tumor$Lineage, levels = c("Classical", "Intermediate", "Basal"))
p <- ggplot(data = ST_case_4_df_tumor, aes(x = Classical, y = Basal, color = Lineage)) + geom_point(shape=16)
p1 <- p + scale_color_manual(values = c("#2F86FFB2", "#EBAB16B2", "#DE0012B2")) + xlab("Classical Score") + ylab("Basal Score") + theme_bw()


#id <- which(ST_case_4_df$Lineage != "NT")
#clone_composition_lineage <- table(ST_case_4_df$Clone[id], ST_case_4_df$Lineage[id])
clone_composition_lineage <- table(ST_case_4_df$Clone, ST_case_4_df$Lineage)
clone_composition_lineage_df <- as.data.frame(matrix(as.vector(clone_composition_lineage), ncol = ncol(clone_composition_lineage)))
rownames(clone_composition_lineage_df) <- rownames(clone_composition_lineage)
colnames(clone_composition_lineage_df) <- colnames(clone_composition_lineage)
clone_composition_lineage_df$node <- c(1:nrow(clone_composition_lineage_df))

lineage.color <- c(piratepal("southpark", trans = .9)[1:3], "grey")
names(lineage.color) <- c("Classical", "Intermediate", "Basal", "NT")

pie_df2 <- nodepie(clone_composition_lineage_df, cols=1:c(ncol(clone_composition_lineage)-1), alpha=0.9, lineage.color ) 	#+ scale_color_manual(values = )

#gp_combo5a + layout_dendrogram()
gp5 <- gp5 +  coord_flip() + xlim(3.3, 0)
gp_combo5a <- inset(gp5, pie_df, height=0.3, width = 0.3, hjust = 3.25 -(min(c(-0.25 - max(dis_node$x) + dis_node$x))) + c(-0.25 - max(dis_node$x) + dis_node$x),  vjust = 0.05)	# +  xlim(NA, max(dis_node$x) + 0.25)
gp_combo5a
gp_combo5b <- inset(gp_combo5a, pie_df2, height=0.4, width = 0.3, hjust = 3.75 -(min(c(-0.25 - max(dis_node$x) + dis_node$x))) + c(-0.25 - max(dis_node$x) + dis_node$x),  vjust = 0.05)	#+  xlim(NA, max(dis_node$x) + 0.25)
gp_combo5b

pdf("Figure 3E.pdf", 12, 6)
print(gp_combo5b)
dev.off()

pdf("Figure 3E_v1.pdf", 12, 2.5)
print(gp5)
dev.off()