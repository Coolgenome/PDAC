setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient6_2")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient6_2/" 
...
png(paste0("./Figure 2_pt2_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_6_p145.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 2394)
Node2 <- SelectingSubTreeData(my.subtrees, 2205)
Node3 <- SelectingSubTreeData(my.subtrees, 2084)
Node4 <- SelectingSubTreeData(my.subtrees, 1613)
Node5 <- SelectingSubTreeData(my.subtrees, 1458)
Node6 <- SelectingSubTreeData(my.subtrees, 1282)
Node7 <- SelectingSubTreeData(my.subtrees, 877)
Node8 <- SelectingSubTreeData(my.subtrees, 745)
Node9 <- SelectingSubTreeData(my.subtrees, 481)												  				
Node10 <- SelectingSubTreeData(my.subtrees, 293)									  
Node11 <- SelectingSubTreeData(my.subtrees, 145)												  				
Node12 <- SelectingSubTreeData(my.subtrees, 3)	

#Merging All Nodes Together
MergedNodes <- rbind(Node1, Node2, Node3, Node4, Node5, Node6, Node7, Node8, Node9, Node10, Node11, Node12)

#Then drop the Histology column
colnames(MergedNodes) = c("Barcode", "Node")

#This then renames "Node" to "Clone"
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_2394", "Clone B2",
					 ifelse(MergedNodes$Node == "Node_2205" , "Clone B4",
                     ifelse(MergedNodes$Node == "Node_2084" , "Clone B3",
					 ifelse(MergedNodes$Node == "Node_1613" , "Clone B5", 
					 ifelse(MergedNodes$Node == "Node_1458" , "Clone B1", 
					 ifelse(MergedNodes$Node == "Node_1282" , "Clone C1", 
					 ifelse(MergedNodes$Node == "Node_877" , "Clone C2", 
					 ifelse(MergedNodes$Node == "Node_745" , "Clone C3", 
					  ifelse(MergedNodes$Node == "Node_481" , "Clone A1", 
					   ifelse(MergedNodes$Node == "Node_293" , "Clone A4", 
					    ifelse(MergedNodes$Node == "Node_145" , "Clone A3", 
                     ifelse(MergedNodes$Node == "Node_3" , "Clone A2", MergedNodes$Node))))))))))))

#Include clone information in the metadata.
ST_case_6@meta.data$Histology_anno_clone <- ST_case_6@meta.data$Histology_anno <- "NT"
ST_case_6@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_6@meta.data))] <- MergedNodes$Node
ST_case_6@meta.data$Clone <- ST_case_6@meta.data$Histology_anno_clone 
ST_case_6@meta.data$Clone[which(ST_case_6@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_6@meta.data$orig.ident <- gsub("Sample145_", "Pt-2", ST_case_6@meta.data$orig.ident)
ST_case_6@meta.data$site[ST_case_6@meta.data$site == "Panc"] <- "Pri"
ST_case_6@meta.data$site[ST_case_6@meta.data$site == "Liver"] <- "LiM"
ST_case_6@meta.data$site[ST_case_6@meta.data$site == "Lung"] <- "LuM"
ST_case_6@meta.data$site[ST_case_6@meta.data$site == "PC"] <- "PerM"
ST_case_6@meta.data$site[ST_case_6@meta.data$orig.ident == "Pt-2A"] <- "Pri"
ST_case_6@meta.data$site[ST_case_6@meta.data$orig.ident == "Pt-2B"] <- "LuM"
ST_case_6@meta.data$site[ST_case_6@meta.data$orig.ident == "Pt-2C"] <- "PerM"

names(ST_case_6@images) <- c(names(table(paste(ST_case_6@meta.data$orig.ident, ST_case_6@meta.data$site))))
Idents(ST_case_6) <- factor(ST_case_6@meta.data$Clone, levels = sort(names(table(ST_case_6@meta.data$Clone))) )
...
clone.color <- table(ST_case_6@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_6$Clone)))
...
gp1 <- flip(gp1, 2697, 3438)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 3439, 3975)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 3977, 4152)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 4308, 4153)

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_6@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <-  c(3176, 2988, 2840, 2698,    5089, 4900, 4779,  4308,  4153, 3977,  3572, 3440 )
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient6_2/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt2_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient6_2/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_6@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_6@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_6@meta.data$Clone, ST_case_6@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt2_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt2_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_6_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_6_tumor <- subset(ST_case_6, idents = names(table(Idents(ST_case_6)))[ grep("Clone", names(table(Idents(ST_case_6))))  ])
ST_case_6_tumor_cnv <- ST_case_6_tumor_cnv[match(rownames(ST_case_6_tumor@meta.data), names(ST_case_6_tumor_cnv))]
length(names(ST_case_6_tumor_cnv) == rownames(ST_case_6_tumor@meta.data))
ST_case_6_tumor@meta.data$InferCNV_score <- ST_case_6_tumor_cnv

Idents(ST_case_6_tumor) <- factor(Idents(ST_case_6_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_6_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt2_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt2_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_6, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
