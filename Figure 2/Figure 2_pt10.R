setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient13_10")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient13_10/" 
...
png(paste0("./Figure 2_pt10_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_13_p142.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 4379)
Node2 <- SelectingSubTreeData(my.subtrees, 4127)
Node3 <- SelectingSubTreeData(my.subtrees, 3918)
Node4 <- SelectingSubTreeData(my.subtrees, 3591)
Node5 <- SelectingSubTreeData(my.subtrees, 3011)
Node6 <- SelectingSubTreeData(my.subtrees, 2645)
Node7 <- SelectingSubTreeData(my.subtrees, 2199)
Node8 <- SelectingSubTreeData(my.subtrees, 1654)
Node9 <- SelectingSubTreeData(my.subtrees, 1508)												  				
Node10 <- SelectingSubTreeData(my.subtrees, 822)									  
Node11 <- SelectingSubTreeData(my.subtrees, 4)		
...
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_4379", "Clone B1",
					 ifelse(MergedNodes$Node == "Node_4127" , "Clone C5",
                     ifelse(MergedNodes$Node == "Node_3918" , "Clone B3",
					 ifelse(MergedNodes$Node == "Node_3591" , "Clone B2", 
					 ifelse(MergedNodes$Node == "Node_3011" , "Clone D1", 
					 ifelse(MergedNodes$Node == "Node_2645" , "Clone A2", 
					 ifelse(MergedNodes$Node == "Node_2199" , "Clone A1", 
					 ifelse(MergedNodes$Node == "Node_1654" , "Clone C1", 
					  ifelse(MergedNodes$Node == "Node_1508" , "Clone C4", 
					   ifelse(MergedNodes$Node == "Node_822" , "Clone C2", 
                     ifelse(MergedNodes$Node == "Node_4" , "Clone C3", MergedNodes$Node)))))))))))
					 
#Include clone information in the metadata.
ST_case_13@meta.data$Histology_anno_clone <- ST_case_13@meta.data$Histology_anno <- "NT"
ST_case_13@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_13@meta.data))] <- MergedNodes$Node
ST_case_13@meta.data$Clone <- ST_case_13@meta.data$Histology_anno_clone 
ST_case_13@meta.data$Clone[which(ST_case_13@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_13@meta.data$orig.ident <- gsub("Sample142_", "Pt-10", ST_case_13@meta.data$orig.ident)
ST_case_13@meta.data$site[ST_case_13@meta.data$site == "Panc"] <- "Pri"
ST_case_13@meta.data$site[ST_case_13@meta.data$site == "Liver"] <- "LiM"
ST_case_13@meta.data$site[ST_case_13@meta.data$site == "Lung"] <- "LuM"
ST_case_13@meta.data$site[ST_case_13@meta.data$site == "PC"] <- "PerM"
ST_case_13@meta.data$site[ST_case_13@meta.data$orig.ident == "Pt-10A"] <- "Pri"
ST_case_13@meta.data$site[ST_case_13@meta.data$orig.ident == "Pt-10B"] <- "LiM"
ST_case_13@meta.data$site[ST_case_13@meta.data$orig.ident == "Pt-10C"] <- "LuM"
ST_case_13@meta.data$site[ST_case_13@meta.data$orig.ident == "Pt-10D"] <- "PerM"

names(ST_case_13@images) <- c(names(table(paste(ST_case_13@meta.data$orig.ident, ST_case_13@meta.data$site))))
Idents(ST_case_13) <- factor(ST_case_13@meta.data$Clone, levels = sort(names(table(ST_case_13@meta.data$Clone))) )
...
clone.color <- table(ST_case_13@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_13$Clone)))
...
row_anno_clean <- ST_case_13@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <- c(7506, 7060, 7872, 9240, 8988, 8779, 8452, 6515, 6369, 5683, 4865)
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient13_10/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt10_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient13_10/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_13@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_13@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_13@meta.data$Clone, ST_case_13@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt10_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt10_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_13_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_13_tumor <- subset(ST_case_13, idents = names(table(Idents(ST_case_13)))[ grep("Clone", names(table(Idents(ST_case_13))))  ])
ST_case_13_tumor_cnv <- ST_case_13_tumor_cnv[match(rownames(ST_case_13_tumor@meta.data), names(ST_case_13_tumor_cnv))]
length(names(ST_case_13_tumor_cnv) == rownames(ST_case_13_tumor@meta.data))
ST_case_13_tumor@meta.data$InferCNV_score <- ST_case_13_tumor_cnv

Idents(ST_case_13_tumor) <- factor(Idents(ST_case_13_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_13_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt10_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt10_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_13, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
