setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient9_7")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient9_7/" 
...
png(paste0("./Figure 2_pt7_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_9_p75.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 9200)
Node2 <- SelectingSubTreeData(my.subtrees, 8708)
Node3 <- SelectingSubTreeData(my.subtrees, 8152)
Node4 <- SelectingSubTreeData(my.subtrees, 7310)
Node5 <- SelectingSubTreeData(my.subtrees, 5916)
Node6 <- SelectingSubTreeData(my.subtrees, 4830)
Node7 <- SelectingSubTreeData(my.subtrees, 2612)
Node8 <- SelectingSubTreeData(my.subtrees, 1568)
Node9 <- SelectingSubTreeData(my.subtrees, 884)												  				
...
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_9200", "Clone A5",
					 ifelse(MergedNodes$Node == "Node_8708" , "Clone A3",
                     ifelse(MergedNodes$Node == "Node_8152" , "Clone A1",
					 ifelse(MergedNodes$Node == "Node_7310" , "Clone A2", 
					 ifelse(MergedNodes$Node == "Node_5916" , "Clone A4", 
					 ifelse(MergedNodes$Node == "Node_4830" , "Clone B1", 
					 ifelse(MergedNodes$Node == "Node_2612" , "Clone C1", 
					 ifelse(MergedNodes$Node == "Node_1568" , "Clone C2", 
					 ifelse(MergedNodes$Node == "Node_884" , "Clone D2", 
                     ifelse(MergedNodes$Node == "Node_4" , "Clone D1", MergedNodes$Node))))))))))

#Include clone information in the metadata.
ST_case_9@meta.data$Histology_anno_clone <- ST_case_9@meta.data$Histology_anno <- "NT"
ST_case_9@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_9@meta.data))] <- MergedNodes$Node
ST_case_9@meta.data$Clone <- ST_case_9@meta.data$Histology_anno_clone 
ST_case_9@meta.data$Clone[which(ST_case_9@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_9@meta.data$orig.ident <- gsub("Sample75_", "Pt-7", ST_case_9@meta.data$orig.ident)
ST_case_9@meta.data$site[ST_case_9@meta.data$site == "Panc"] <- "Pri"
ST_case_9@meta.data$site[ST_case_9@meta.data$site == "Liver"] <- "LiM"
ST_case_9@meta.data$site[ST_case_9@meta.data$site == "Lung"] <- "LuM"
ST_case_9@meta.data$site[ST_case_9@meta.data$site == "PC"] <- "PerM"
ST_case_9@meta.data$site[ST_case_9@meta.data$orig.ident == "Pt-7A"] <- "Pri"
ST_case_9@meta.data$site[ST_case_9@meta.data$orig.ident == "Pt-7B"] <- "LiM-1"
ST_case_9@meta.data$site[ST_case_9@meta.data$orig.ident == "Pt-7C"] <- "LiM-2"
ST_case_9@meta.data$site[ST_case_9@meta.data$orig.ident == "Pt-7D"] <- "PerM"

names(ST_case_9@images) <- c(names(table(paste(ST_case_9@meta.data$orig.ident, ST_case_9@meta.data$site))))
Idents(ST_case_9) <- factor(ST_case_9@meta.data$Clone, levels = sort(names(table(ST_case_9@meta.data$Clone))) )
...
clone.color <- table(ST_case_9@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_9$Clone)))
...
gp1 <- flip(gp1, 14314, 9487)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 15401, 16794)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 18685, 18193)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 10369, 9489)

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_9@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <- c(15401, 18685, 18193, 17637,  16795, 14315,  12097, 11053, 10369, 9489)
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient9_7/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt7_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient9_7/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_9@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_9@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_9@meta.data$Clone, ST_case_9@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt7_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt7_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_9_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_9_tumor <- subset(ST_case_9, idents = names(table(Idents(ST_case_9)))[ grep("Clone", names(table(Idents(ST_case_9))))  ])
ST_case_9_tumor_cnv <- ST_case_9_tumor_cnv[match(rownames(ST_case_9_tumor@meta.data), names(ST_case_9_tumor_cnv))]
length(names(ST_case_9_tumor_cnv) == rownames(ST_case_9_tumor@meta.data))
ST_case_9_tumor@meta.data$InferCNV_score <- ST_case_9_tumor_cnv

Idents(ST_case_9_tumor) <- factor(Idents(ST_case_9_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_9_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt7_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt7_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_9, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
