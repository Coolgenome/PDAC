setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient8_6")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient8_6/" 
...
png(paste0("./Figure 2_pt6_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_8_p27.Rdata")


Node1 <- SelectingSubTreeData(my.subtrees, 1983)
Node2 <- SelectingSubTreeData(my.subtrees, 1748)
Node3 <- SelectingSubTreeData(my.subtrees, 1456)
Node4 <- SelectingSubTreeData(my.subtrees, 1074)
Node5 <- SelectingSubTreeData(my.subtrees, 833)
Node6 <- SelectingSubTreeData(my.subtrees, 639)
Node7 <- SelectingSubTreeData(my.subtrees, 426)
Node8 <- SelectingSubTreeData(my.subtrees, 213)
Node9 <- SelectingSubTreeData(my.subtrees, 98)												  				
...
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_1983", "Clone D3",
					 ifelse(MergedNodes$Node == "Node_1748" , "Clone A2",
                     ifelse(MergedNodes$Node == "Node_1456" , "Clone A3",
					 ifelse(MergedNodes$Node == "Node_1074" , "Clone D1", 
					 ifelse(MergedNodes$Node == "Node_833" , "Clone D2", 
					 ifelse(MergedNodes$Node == "Node_639" , "Clone C1", 
					 ifelse(MergedNodes$Node == "Node_426" , "Clone B2", 
					 ifelse(MergedNodes$Node == "Node_213" , "Clone B1", 
					 ifelse(MergedNodes$Node == "Node_98" , "Clone A1", 
                     ifelse(MergedNodes$Node == "Node_3" , "Clone B3", MergedNodes$Node))))))))))


#Include clone information in the metadata.
ST_case_8@meta.data$Histology_anno_clone <- ST_case_8@meta.data$Histology_anno <- "NT"
ST_case_8@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_8@meta.data))] <- MergedNodes$Node
ST_case_8@meta.data$Clone <- ST_case_8@meta.data$Histology_anno_clone 
ST_case_8@meta.data$Clone[which(ST_case_8@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_8@meta.data$orig.ident <- gsub("Sample27_", "Pt-6", ST_case_8@meta.data$orig.ident)
ST_case_8@meta.data$site[ST_case_8@meta.data$site == "Panc"] <- "Pri"
ST_case_8@meta.data$site[ST_case_8@meta.data$site == "Liver"] <- "LiM"
ST_case_8@meta.data$site[ST_case_8@meta.data$site == "Lung"] <- "LuM"
ST_case_8@meta.data$site[ST_case_8@meta.data$site == "PC"] <- "PerM"
ST_case_8@meta.data$site[ST_case_8@meta.data$orig.ident == "Pt-6A"] <- "Pri"
ST_case_8@meta.data$site[ST_case_8@meta.data$orig.ident == "Pt-6B"] <- "LiM"
ST_case_8@meta.data$site[ST_case_8@meta.data$orig.ident == "Pt-6C"] <- "PerM-1"
ST_case_8@meta.data$site[ST_case_8@meta.data$orig.ident == "Pt-6D"] <- "PerM-2"
names(ST_case_8@images) <- c(names(table(paste(ST_case_8@meta.data$orig.ident, ST_case_8@meta.data$site))))
Idents(ST_case_8) <- factor(ST_case_8@meta.data$Clone, levels = sort(names(table(ST_case_8@meta.data$Clone))) )
...
clone.color <- table(ST_case_8@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_8$Clone)))
...
gp1 <- flip(gp1, 4078, 3843)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 2521, 2192)

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_8@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <-  c(2193, 3843, 3551, 2308, 2521, 2098, 2734,	3169,  2928,   4078 )	
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient8_6/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt6_heatmap_raw.png") 	)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_8@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_8@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_8@meta.data$Clone, ST_case_8@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt6_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt6_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_8_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_8_tumor <- subset(ST_case_8, idents = names(table(Idents(ST_case_8)))[ grep("Clone", names(table(Idents(ST_case_8))))  ])
ST_case_8_tumor_cnv <- ST_case_8_tumor_cnv[match(rownames(ST_case_8_tumor@meta.data), names(ST_case_8_tumor_cnv))]
length(names(ST_case_8_tumor_cnv) == rownames(ST_case_8_tumor@meta.data))
ST_case_8_tumor@meta.data$InferCNV_score <- ST_case_8_tumor_cnv

Idents(ST_case_8_tumor) <- factor(Idents(ST_case_8_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_8_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt6_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt6_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_8, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
