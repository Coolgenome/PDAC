setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient7_12")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient7_12/" 
...
png(paste0("./Figure 2_pt12_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_7_p3.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 3140)
Node11 <- SelectingSubTreeData(my.subtrees, 2901)
Node2 <- SelectingSubTreeData(my.subtrees, 2710)
Node3 <- SelectingSubTreeData(my.subtrees, 2057)
Node4 <- SelectingSubTreeData(my.subtrees, 1998)
Node5 <- SelectingSubTreeData(my.subtrees, 1613)
Node6 <- SelectingSubTreeData(my.subtrees, 1108)
Node7 <- SelectingSubTreeData(my.subtrees, 591)
Node8 <- SelectingSubTreeData(my.subtrees, 410)												  				
Node9 <- SelectingSubTreeData(my.subtrees, 210)									  
Node10 <- SelectingSubTreeData(my.subtrees, 3)									  

#Merging All Nodes Together
MergedNodes <- rbind(Node1, Node11, Node2, Node3, Node4, Node5, Node6, Node7, Node8, Node9, Node10)
...
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_3140", "Clone B2",
					 ifelse(MergedNodes$Node == "Node_2901" , "Clone B1",
                     ifelse(MergedNodes$Node == "Node_2710" , "Clone B3",
					 ifelse(MergedNodes$Node == "Node_2057" , "Clone D1", 
					 ifelse(MergedNodes$Node == "Node_1998" , "Clone A", 
					 ifelse(MergedNodes$Node == "Node_1613" , "Clone D2", 
					 ifelse(MergedNodes$Node == "Node_1108" , "Clone C4", 
					 ifelse(MergedNodes$Node == "Node_591" , "Clone C5", 
					 ifelse(MergedNodes$Node == "Node_410" , "Clone C3", 
				     ifelse(MergedNodes$Node == "Node_210" , "Clone C2", 
                     ifelse(MergedNodes$Node == "Node_3" , "Clone C1", MergedNodes$Node)))))))))))

#Include clone information in the metadata.
ST_case_7@meta.data$Histology_anno_clone <- ST_case_7@meta.data$Histology_anno <- "NT"
ST_case_7@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_7@meta.data))] <- MergedNodes$Node
ST_case_7@meta.data$Clone <- ST_case_7@meta.data$Histology_anno_clone 
ST_case_7@meta.data$Clone[which(ST_case_7@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_7@meta.data$orig.ident <- gsub("Sample3_", "Pt-12", ST_case_7@meta.data$orig.ident)
ST_case_7@meta.data$site[ST_case_7@meta.data$site == "Panc"] <- "Pri"
ST_case_7@meta.data$site[ST_case_7@meta.data$site == "Liver"] <- "LiM"
ST_case_7@meta.data$site[ST_case_7@meta.data$site == "Lung"] <- "LuM"
ST_case_7@meta.data$site[ST_case_7@meta.data$site == "PC"] <- "PerM"
ST_case_7@meta.data$site[ST_case_7@meta.data$orig.ident == "Pt-12A"] <- "Pri"
ST_case_7@meta.data$site[ST_case_7@meta.data$orig.ident == "Pt-12B"] <- "LiM"
ST_case_7@meta.data$site[ST_case_7@meta.data$orig.ident == "Pt-12C"] <- "LuM"
ST_case_7@meta.data$site[ST_case_7@meta.data$orig.ident == "Pt-12D"] <- "PerM"
names(ST_case_7@images) <- c(names(table(paste(ST_case_7@meta.data$orig.ident, ST_case_7@meta.data$site))))
Idents(ST_case_7) <- factor(ST_case_7@meta.data$Clone, levels = sort(names(table(ST_case_7@meta.data$Clone))) )
...
clone.color <- table(ST_case_7@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_7$Clone)))
...
gp1 <- flip(gp1, 5473, 5532)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 4583, 4066)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 3885, 3685)


#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_7@meta.data[,c("site", "Clone")]
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient7_12/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt12_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient7_12/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_7@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_7@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_7@meta.data$Clone, ST_case_7@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt12_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt12_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_7_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_7_tumor <- subset(ST_case_7, idents = names(table(Idents(ST_case_7)))[ grep("Clone", names(table(Idents(ST_case_7))))  ])
ST_case_7_tumor_cnv <- ST_case_7_tumor_cnv[match(rownames(ST_case_7_tumor@meta.data), names(ST_case_7_tumor_cnv))]
length(names(ST_case_7_tumor_cnv) == rownames(ST_case_7_tumor@meta.data))
ST_case_7_tumor@meta.data$InferCNV_score <- ST_case_7_tumor_cnv

Idents(ST_case_7_tumor) <- factor(Idents(ST_case_7_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_7_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt12_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt12_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_7, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
