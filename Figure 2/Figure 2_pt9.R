setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient10_9")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient10_9/" 
...
png(paste0("./Figure 2_pt9_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_10_p83.Rdata")

Node1 <- SelectingSubTreeData(my.subtrees, 5032)
Node2 <- SelectingSubTreeData(my.subtrees, 4941)
Node3 <- SelectingSubTreeData(my.subtrees, 4100)
Node4 <- SelectingSubTreeData(my.subtrees, 3148)
Node5 <- SelectingSubTreeData(my.subtrees, 2819)
Node6 <- SelectingSubTreeData(my.subtrees, 1853)
Node7 <- SelectingSubTreeData(my.subtrees, 1275)
Node8 <- SelectingSubTreeData(my.subtrees, 671)
Node9 <- SelectingSubTreeData(my.subtrees, 109)												  				
Node10 <- SelectingSubTreeData(my.subtrees, 4)						

#Merging All Nodes Together
MergedNodes <- rbind(Node1, Node2, Node3, Node4, Node5, Node6, Node7, Node8, Node9, Node10)

#Then drop the Histology column
colnames(MergedNodes) = c("Barcode", "Node")

#This then renames "Node" to "Clone"
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_5032", "Clone A3",
					 ifelse(MergedNodes$Node == "Node_4941" , "Clone A2",
                     ifelse(MergedNodes$Node == "Node_4100" , "Clone A4",
					 ifelse(MergedNodes$Node == "Node_3148" , "Clone A1", 
					 ifelse(MergedNodes$Node == "Node_2819" , "Clone C2", 
					 ifelse(MergedNodes$Node == "Node_1853" , "Clone C1", 
					 ifelse(MergedNodes$Node == "Node_1275" , "Clone B4", 
					 ifelse(MergedNodes$Node == "Node_671" , "Clone B3", 
					 ifelse(MergedNodes$Node == "Node_109" , "Clone B1", 
                     ifelse(MergedNodes$Node == "Node_4" , "Clone B2", MergedNodes$Node))))))))))

#Include clone information in the metadata.
ST_case_10@meta.data$Histology_anno_clone <- ST_case_10@meta.data$Histology_anno <- "NT"
ST_case_10@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_10@meta.data))] <- MergedNodes$Node
ST_case_10@meta.data$Clone <- ST_case_10@meta.data$Histology_anno_clone 
ST_case_10@meta.data$Clone[which(ST_case_10@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_10@meta.data$orig.ident <- gsub("Sample83_", "Pt-9", ST_case_10@meta.data$orig.ident)
ST_case_10@meta.data$site[ST_case_10@meta.data$site == "Panc"] <- "Pri"
ST_case_10@meta.data$site[ST_case_10@meta.data$site == "Liver"] <- "LiM"
ST_case_10@meta.data$site[ST_case_10@meta.data$site == "Lung"] <- "LuM"
ST_case_10@meta.data$site[ST_case_10@meta.data$site == "PC"] <- "PerM"
ST_case_10@meta.data$site[ST_case_10@meta.data$orig.ident == "Pt-9A"] <- "Pri"
ST_case_10@meta.data$site[ST_case_10@meta.data$orig.ident == "Pt-9B"] <- "LiM-1"
ST_case_10@meta.data$site[ST_case_10@meta.data$orig.ident == "Pt-9C"] <- "LiM-2"
ST_case_10@meta.data$site[ST_case_10@meta.data$orig.ident == "Pt-9D"] <- "PerM"

names(ST_case_10@images) <- c(names(table(paste(ST_case_10@meta.data$orig.ident, ST_case_10@meta.data$site))))
Idents(ST_case_10) <- factor(ST_case_10@meta.data$Clone, levels = sort(names(table(ST_case_10@meta.data$Clone))) )
...
clone.color <- table(ST_case_10@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_10$Clone)))
...
gp1 <- flip(gp1, 10505, 9665)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 8384, 7418)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 6840, 6236)

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_10@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <- c(10597, 10506, 9665, 8713,   6840, 6236, 5674, 5569,  8384, 7418)
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient10_9/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt9_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient10_9/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_10@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_10@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_10@meta.data$Clone, ST_case_10@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt9_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt9_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_10_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_10_tumor <- subset(ST_case_10, idents = names(table(Idents(ST_case_10)))[ grep("Clone", names(table(Idents(ST_case_10))))  ])
ST_case_10_tumor_cnv <- ST_case_10_tumor_cnv[match(rownames(ST_case_10_tumor@meta.data), names(ST_case_10_tumor_cnv))]
length(names(ST_case_10_tumor_cnv) == rownames(ST_case_10_tumor@meta.data))
ST_case_10_tumor@meta.data$InferCNV_score <- ST_case_10_tumor_cnv

Idents(ST_case_10_tumor) <- factor(Idents(ST_case_10_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_10_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt9_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt9_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_10, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
...
DefaultAssay(ST_case_10) <- "Spatial"
ST_case_10 <- AddModuleScore(ST_case_10, features = list(tumor_marker$scClassical), 	name = "Classical", replace = TRUE)
ST_case_10 <- AddModuleScore(ST_case_10, features = list(tumor_marker$scBasal),       	name = "Basal", replace = TRUE)

ST_case_10_df <- data.frame(cbind(ST_case_10$Classical1, ST_case_10$Basal1, "NT", ST_case_10$Clone))
colnames(ST_case_10_df) <- c("Classical", "Basal", "Lineage", "Clone")

id1 <- which(ST_case_10$Basal1 < ST_case_10$Classical1 - 0.3)	#, which(as.numeric(ST_case_10$Classical1) > 0.2))
id2 <- which(ST_case_10$Basal1 > ST_case_10$Classical1 + 0.3)	#, which(as.numeric(ST_case_10$Basal1) > 0.2))
id3 <- intersect(which(ST_case_10$Basal1 < ST_case_10$Classical1 + 0.3), which(ST_case_10$Basal1 > ST_case_10$Classical1 - 0.3))
ST_case_10_df$Lineage[id1] <- "Classical"
ST_case_10_df$Lineage[id2] <- "Basal"
ST_case_10_df$Lineage[id3] <- "Intermediate"
ST_case_10_df$Lineage[grep("Clone", ST_case_10_df$Clone, invert = T)] <- "NT"

ST_case_10_df_tumor <- ST_case_10_df[grep("Clone", ST_case_10_df$Clone),]
ST_case_10_df_tumor$Classical <- as.numeric(ST_case_10_df_tumor$Classical)
ST_case_10_df_tumor$Basal <- as.numeric(ST_case_10_df_tumor$Basal)
ST_case_10_df_tumor$Lineage <- factor(ST_case_10_df_tumor$Lineage, levels = c("Classical", "Intermediate", "Basal"))
p <- ggplot(data = ST_case_10_df_tumor, aes(x = Classical, y = Basal, color = Lineage)) + geom_point(shape=16)
p1 <- p + scale_color_manual(values = c("#2F86FFB2", "#EBAB16B2", "#DE0012B2")) + xlab("Classical Score") + ylab("Basal Score") + theme_bw()


#id <- which(ST_case_10_df$Lineage != "NT")
#clone_composition_lineage <- table(ST_case_10_df$Clone[id], ST_case_10_df$Lineage[id])
clone_composition_lineage <- table(ST_case_10_df$Clone, ST_case_10_df$Lineage)
...
gp5 <- gp5 +  coord_flip() + xlim(1.4,0)
gp_combo5a <- inset(gp5, pie_df, height=0.3, width = 0.3, hjust = 1.2 -(min(c(-0.25 - max(dis_node$x) + dis_node$x))) + c(-0.25 - max(dis_node$x) + dis_node$x),  vjust = 0.05)	# +  xlim(NA, max(dis_node$x) + 0.25)
gp_combo5a
gp_combo5b <- inset(gp_combo5a, pie_df2, height=0.3, width = 0.3, hjust = 1.4 -(min(c(-0.25 - max(dis_node$x) + dis_node$x))) + c(-0.25 - max(dis_node$x) + dis_node$x),  vjust = 0.05)	#+  xlim(NA, max(dis_node$x) + 0.25)
gp_combo5b

pdf("Figure 3C.pdf", 12, 6)
print(gp_combo5b)
dev.off()

pdf("Figure 3C_v1.pdf", 12, 3.5)
print(gp5)
dev.off()
