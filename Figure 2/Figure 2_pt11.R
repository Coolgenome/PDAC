setwd("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient1_11")
...
input_path <- "C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient1_11/" 
...
png(paste0("./Figure 2_pt11_phylo.png"), width = 3500, height = 2500, res = 300)
...
st <- load("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/ST_case_1_p52.Rdata")

#Manual clone assignment
Node1 <- SelectingSubTreeData(my.subtrees, 3)			
Node2 <- SelectingSubTreeData(my.subtrees, 217)
Node21 <- SelectingSubTreeData(my.subtrees, 375)

Node3 <- SelectingSubTreeData(my.subtrees, 657)								  
Node4 <- SelectingSubTreeData(my.subtrees, 1122)												  				
Node5 <- SelectingSubTreeData(my.subtrees, 1247)					
						  			
Node6 <- SelectingSubTreeData(my.subtrees, 1385)
Node61 <- SelectingSubTreeData(my.subtrees, 1461)					
Node62 <- SelectingSubTreeData(my.subtrees, 1547)		
Node7 <- SelectingSubTreeData(my.subtrees, 1765)	
									
Node8 <- SelectingSubTreeData(my.subtrees, 2123)				
Node9 <- SelectingSubTreeData(my.subtrees, 2702)	
Node10 <- SelectingSubTreeData(my.subtrees, 2892)	
Node11 <- SelectingSubTreeData(my.subtrees, 2976)	

#Merging All Nodes Together
MergedNodes <- rbind(Node1, Node2, Node21, Node3, Node4, Node5, Node6, Node61, Node62, Node7, Node8, Node9, Node10, Node11)

#Then drop the Histology column
colnames(MergedNodes) = c("Barcode", "Node")

#This then renames "Node" to "Clone"
MergedNodes$Node <- ifelse(MergedNodes$Node == "Node_3", "Clone C5",
                     ifelse(MergedNodes$Node == "Node_217" , "Clone C4",
					 ifelse(MergedNodes$Node == "Node_375" , "Clone C3",
					 
					 ifelse(MergedNodes$Node == "Node_657" , "Clone A1", 
					 ifelse(MergedNodes$Node == "Node_1122" , "Clone A3", 
					 ifelse(MergedNodes$Node == "Node_1247" , "Clone A2", 
					 
					 ifelse(MergedNodes$Node == "Node_1385" , "Clone B2",
					 ifelse(MergedNodes$Node == "Node_1461" , "Clone B1",
					 ifelse(MergedNodes$Node == "Node_1547" , "Clone B4",
					 ifelse(MergedNodes$Node == "Node_1765" , "Clone B3",
					 
					 ifelse(MergedNodes$Node == "Node_2123" , "Clone C1",
					 ifelse(MergedNodes$Node == "Node_2702" , "Clone D1",
					 ifelse(MergedNodes$Node == "Node_2892" , "Clone C2",
                     ifelse(MergedNodes$Node == "Node_2976" , "Clone D2", MergedNodes$Node))))))))))))))

#Include clone information in the metadata.
ST_case_1@meta.data$Histology_anno_clone <- ST_case_1@meta.data$Histology_anno <- "NT"
ST_case_1@meta.data$Histology_anno_clone[match(MergedNodes$Barcode, rownames(ST_case_1@meta.data))] <- MergedNodes$Node
ST_case_1@meta.data$Clone <- ST_case_1@meta.data$Histology_anno_clone 
ST_case_1@meta.data$Clone[which(ST_case_1@meta.data$CNV_cluster == "NT")] <- "NT"
...
#Information update
ST_case_1@meta.data$orig.ident <- gsub("Sample52_", "Pt-11", ST_case_1@meta.data$orig.ident)
ST_case_1@meta.data$site[ST_case_1@meta.data$site == "Panc"] <- "Pri"
ST_case_1@meta.data$site[ST_case_1@meta.data$site == "Liver"] <- "LiM"
ST_case_1@meta.data$site[ST_case_1@meta.data$site == "Lung"] <- "LuM"
ST_case_1@meta.data$site[ST_case_1@meta.data$site == "PC"] <- "PerM"
ST_case_1@meta.data$site[ST_case_1@meta.data$orig.ident == "Pt-11A"] <- "Pri"
ST_case_1@meta.data$site[ST_case_1@meta.data$orig.ident == "Pt-11B"] <- "LiM"
ST_case_1@meta.data$site[ST_case_1@meta.data$orig.ident == "Pt-11C"] <- "LuM"
ST_case_1@meta.data$site[ST_case_1@meta.data$orig.ident == "Pt-11D"] <- "PerM"
names(ST_case_1@images) <- c(names(table(paste(ST_case_1@meta.data$orig.ident, ST_case_1@meta.data$site))))
Idents(ST_case_1) <- factor(ST_case_1@meta.data$Clone, levels = sort(names(table(ST_case_1@meta.data$Clone))) )
...
clone.color <- table(ST_case_1@meta.data$Clone)							
...
names(clone.color) <- c(names(table(ST_case_1$Clone)))
...
gp1 <- flip(gp1, 3847, 4574)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 4575, 5313)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 5892, 5314)
gp1 + geom_text(aes(label = node))
gp1 <- flip(gp1, 5893, 6082)
gp1

#Calculate the tissue composition for each subclone
row_anno_clean <- ST_case_1@meta.data[,c("site", "Clone")]
...
clone_composition_df$node <- c(3848, 4438, 4313, 	4956, 4738, 4652, 4576, 	5314, 3566, 3408, 3194,6083 ,	5893, 6167)
...
data1 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient1_11/infercnv.observations.txt", sep = " ", row.names = 1)
...
pheatmap(data_clean_map[spot_order1,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 5,
		 cutree_col = 2,
		 file = paste0("Figure 2_pt11_heatmap_raw.png") 	)
...
data2 = read.delim("C:/MD_Anderson/4.PDAC_ST/6.SpatialInferCNV_by_patient_manual_anno/New_inferCNV_core_4/InferCNV_outputs_patient1_11/infercnv.references.txt", sep = " ", row.names = 1)
...
dot_size <- 3 * (log(as.numeric(table(ST_case_1@meta.data$Clone)[grep("Clone|NT", names(table(ST_case_1@meta.data$Clone)))]))) - 3
...
temp <- table(ST_case_1@meta.data$Clone, ST_case_1@meta.data$site)[c("NT"),]
...
pdf("Figure 2_pt11_part1.pdf", 6, 8)
print(gp_combo5a)
dev.off()
...
pheatmap(data_merge_map[spot_order,], scale = 'none', color = colorRampPalette(c("navy", "white", "firebrick"))(50), angle_col = "90", 
         ...
         height = 3.5,
		 cutree_col = 2, 
		 file = paste0("Figure 2_pt11_part2.png") 	)

# CNV score 
data1_clean <- data1
data1_clean[abs(data1_clean - 1) < 0.02] <- 1
ST_case_1_tumor_cnv <- apply(data1_clean, 2, sd)

ST_case_1_tumor <- subset(ST_case_1, idents = names(table(Idents(ST_case_1)))[ grep("Clone", names(table(Idents(ST_case_1))))  ])
ST_case_1_tumor_cnv <- ST_case_1_tumor_cnv[match(rownames(ST_case_1_tumor@meta.data), names(ST_case_1_tumor_cnv))]
length(names(ST_case_1_tumor_cnv) == rownames(ST_case_1_tumor@meta.data))
ST_case_1_tumor@meta.data$InferCNV_score <- ST_case_1_tumor_cnv

Idents(ST_case_1_tumor) <- factor(Idents(ST_case_1_tumor), levels = rev(spot_order))
p0 <- VlnPlot(ST_case_1_tumor, features = c("InferCNV_score"), pt.size=0) + theme(legend.position = "none") + xlab("")
...
pdf("Figure 2_pt11_part3.pdf", 1.5, 4)	 
print (p5)
dev.off()

clone.color["NT"] <- "white"

pdf("Figure 2_pt11_part4.pdf", 16, 5)
SpatialDimPlot(ST_case_1, label = FALSE, label.size = 3, alpha = 1, cols = clone.color, stroke = 0.01, pt.size.factor = 5000)
dev.off()
